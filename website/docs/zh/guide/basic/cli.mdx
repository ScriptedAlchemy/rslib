...
| `--project <name>`         | 指定运行命令的工作区项目（可重复，支持通配符和取反，仅在配置 [projects](/config/projects) 时可用）                        |
...
在工作区模式下使用 `--project` 时，Rslib 会在匹配前自动去除每个值两侧的空白字符。  
每个值必须是非空字符串，且取反模式需要在 `!` 后提供非空模式（例如 `!@scope/legacy`）。
...
:::note
当配置了 [projects](/config/projects) 工作区模式时，`rslib build` 会统一编排所有匹配到的子项目，并按依赖顺序构建。  
如果 `--project` 过滤后没有匹配到任何工作区项目（包括仅使用取反规则导致全部被排除的情况），Rslib 会报告过滤未命中，并输出可用的工作区项目名。
该错误里输出的可用工作区项目名会按稳定的排序顺序展示。
如果传入了重复的 `--project` 规则，过滤未命中的错误信息会仅输出规范化、去重后且保持首次出现顺序的规则。
工作区模式暂不支持 `build --watch`。
:::
...
:::note
当配置了 [projects](/config/projects) 工作区模式时，`rslib inspect` 会对每个匹配到的子项目执行。
如果 `--project` 过滤后没有匹配到任何工作区项目，Rslib 会报告过滤未命中，并输出可用的工作区项目名。
该错误里输出的可用工作区项目名会按稳定的排序顺序展示。
如果传入了重复的 `--project` 规则，过滤未命中的错误信息会仅输出规范化、去重后且保持首次出现顺序的规则。
:::
...
:::note
`rslib mf-dev` 目前仅支持单项目模式。  
当配置了 [projects](/config/projects) 工作区模式后，暂不支持 `mf-dev`。
在工作区模式下，如果传入 `--project` 且没有匹配到任何项目，Rslib 会报告过滤未命中并输出可用的工作区项目名。
该错误里输出的可用工作区项目名会按稳定的排序顺序展示。
如果传入了重复的 `--project` 规则，过滤未命中的错误信息会仅输出规范化、去重后且保持首次出现顺序的规则。
:::
