...
The type of workspace orchestration configuration when using [projects](/config/projects).

```ts twoslash
import type { RslibWorkspaceConfig } from '@rslib/core';

const config: RslibWorkspaceConfig = {
  projects: ['packages/*'],
};
```
...
Union type of `RslibConfig` and `RslibWorkspaceConfig`.

```ts twoslash
import type { RslibUserConfig } from '@rslib/core';
```
