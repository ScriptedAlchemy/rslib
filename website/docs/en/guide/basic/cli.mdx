...
| `--project <name>`         | Specify workspace project(s) to run commands (repeatable, supports wildcard and negation, only available when [projects](/config/projects) is configured)   |
...
When using `--project` in workspace mode, Rslib trims each value before matching.  
Each value must be non-empty, and negation values must include a non-empty pattern after `!` (for example, `!@scope/legacy`).
...
:::note
When [projects](/config/projects) is configured for workspace mode, `rslib build` orchestrates all matched child projects and builds them in dependency order.  
If `--project` filters match no workspace projects (including negation-only filters that exclude everything), Rslib reports the filter miss and prints available workspace project names.
Those available workspace project names are listed in deterministic sorted order.
If repeated `--project` filters are provided, the filter-miss diagnostics print normalized unique patterns only, preserving first-occurrence order.
In workspace mode, `build --watch` is not supported yet.
:::
...
:::note
When [projects](/config/projects) is configured for workspace mode, `rslib inspect` runs for each matched child project.
If `--project` filters match no workspace projects, Rslib reports the filter miss and prints available workspace project names.
Those available workspace project names are listed in deterministic sorted order.
If repeated `--project` filters are provided, the filter-miss diagnostics print normalized unique patterns only, preserving first-occurrence order.
:::
...
:::note
`rslib mf-dev` is currently only available for single-project mode.  
When [projects](/config/projects) is configured in workspace mode, `mf-dev` is not supported yet.
If you pass `--project` in workspace mode and no projects match, Rslib reports the filter miss and lists available workspace project names.
Those available workspace project names are listed in deterministic sorted order.
If repeated `--project` filters are provided, the filter-miss diagnostics print normalized unique patterns only, preserving first-occurrence order.
:::
