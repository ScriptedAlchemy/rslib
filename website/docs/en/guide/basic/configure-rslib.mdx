...
You can also use Rslib in a monorepo workspace by defining [projects](/config/projects):
...
In this mode, Rslib will resolve child projects and orchestrate `build` / `inspect` across them.  
`rslib build` will sort projects by local dependency relationships before building.

:::note

- `projects` and `lib` cannot be used in the same config file.
- Workspace mode currently supports `build` and `inspect`.
- `build --watch` and `mf-dev` are not supported in workspace mode yet.
  :::
